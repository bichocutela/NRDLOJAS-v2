...
internal class AcpApi(private val store: AcpStorage, clientBuilder: OkHttpClient.Builder = OkHttpClient.Builder(),
    ...
    suspend fun configure(login: String, password: String) = withContext(Dispatchers.IO) {
        ...
        store.write("access", JSONObject().put("login", login.trim()).put("password", password).toString()); cookies.clear()
    }
    suspend fun confirmAccess() = withContext(Dispatchers.IO) {
        try { session() } catch (_: AcpUnauthorized) {
            val access = credentials() ?: throw AcpFailure("Peça ao administrador para configurar o acesso neste aparelho.")
            cookies.clear()
            val csrf = requestJson(Request.Builder().url("$ORIGIN/api/auth/csrf").get().build()).nonBlankString("csrfToken")
                ?: throw AcpFailure("A ACP não disponibilizou o formulário de acesso. Tente novamente.")
            val form = FormBody.Builder().add("login", access.getString("login")).add("password", access.getString("password"))
                .add("csrfToken", csrf).add("callbackUrl", "$ORIGIN/print-template").add("json", "true").build()
            ...
            val error = result.nonBlankString("error") ?: result.nonBlankString("url")?.let { ORIGIN.toHttpUrl().resolve(it)?.queryParameter("error") }
            if (!error.isNullOrBlank()) { cookies.clear(); throw AcpFailure("A ACP não aceitou o acesso. Peça ao administrador para conferir a configuração.") }
            session()
        }
    }
    ...
    private fun credentials(): JSONObject? {
        ...
        return store.read("access")?.let { runCatching { JSONObject(it) }.getOrNull() }
    ...
    private fun session(): JSONObject {
        ...
        if (user == null || user.nonBlankString("accessToken") == null) { cookies.clear(); throw AcpUnauthorized() }
        return user
    }

    /** ACP integration is deliberately read-only. No campaign/product mutation endpoint is accepted here. */
    ...
    private fun requestJson(request: Request): JSONObject {
        client.newCall(request).execute().use { response ->
            if (response.code == 401 || (response.code in 300..399 && response.header("Location").orEmpty().contains("/login"))) {
                cookies.clear()
                if (request.url.encodedPath == "/api/auth/callback/credentials") throw AcpFailure("A ACP não aceitou o acesso. Peça ao administrador para conferir a configuração.")
            ...
            return try { JSONObject(raw) } catch (_: Exception) { throw AcpFailure("A resposta da ACP não é compatível com esta consulta.") }
        }
    }
    private fun JSONObject.nonBlankString(name: String): String? = (opt(name) as? String)?.takeIf { it.isNotBlank() && it != "null" }
    companion object {
...
internal class AcpCookieJar(saved: String?, private val persist: (String) -> Unit) : CookieJar {
    private val origin = AcpApi.ORIGIN.toHttpUrl(); private val values = mutableListOf<Cookie>()
    init { runCatching { val array = JSONArray(saved ?: "[]"); for (index in 0 until array.length()) Cookie.parse(origin, array.getString(index))?.let { values.add(it) } } }
    @Synchronized override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
        if (url.host != origin.host || !url.isHttps) return
        cookies.forEach { cookie -> values.removeAll { it.name == cookie.name && it.domain == cookie.domain && it.path == cookie.path }; if (cookie.expiresAt > System.currentTimeMillis()) values.add(cookie) }
        persist(JSONArray(values.map { it.toString() }).toString())
    }
    @Synchronized override fun loadForRequest(url: HttpUrl): List<Cookie> = if (url.host != origin.host || !url.isHttps) emptyList() else values.filter { it.expiresAt > System.currentTimeMillis() && it.matches(url) }
    @Synchronized fun clear() { values.clear(); persist("[]") }
}
